...
namespace Restful.Legacy
{
	...
	public class RestManager
	{
		...
		{
			// Server Commands
			if (Restful.Instance.Config.EnableTokenEndpointAuthentication)
			{
		...
		{
			string username = args.Parameters["user"];
		...
		{
			if (!Boolean.TryParse(args.Verbs["state"], out bool autoSave))
		...
		{
			if (!Boolean.TryParse(args.Parameters["state"], out bool autoSave))
			{
		...
		{
			if (!Boolean.TryParse(args.Parameters["killfriendly"], out bool killFriendly))
		...
		{
			if (!Boolean.TryParse(args.Verbs["bloodmoon"], out bool bloodmoon))
		...
		{
			if (!Boolean.TryParse(args.Verbs["state"], out bool bloodmoon))
			{
		...
		{
			List<Player> activeplayers = Main.player.Where(p => null != p && p.active).ToList();
		...
		private object PlayerListV2(RestRequestArgs args)
		{
			...
			foreach (TSPlayer tsPlayer in TShock.Players.Where(p => null != p))
			{
				Dictionary<string, object> p = PlayerFilter(tsPlayer, args.Parameters);
				if (null != p)
					playerList.Add(p);
			}
		...
		private object PlayerReadV3(RestRequestArgs args)
		{
			...
			List<Item> inventory = player.TPlayer.inventory.Where(p => p.active).ToList();
			List<Item> equipment = player.TPlayer.armor.Where(p => p.active).ToList();
			List<Item> dyes = player.TPlayer.dye.Where(p => p.active).ToList();
			return new RestObject()
			{
		...
		private object PlayerBanV2(RestRequestArgs args)
		{
			...
			TSPlayer player = (TSPlayer)ret;
			string reason = null == args.Parameters["reason"] ? "Banned via web" : args.Parameters["reason"];
		...
		private object PlayerKill(RestRequestArgs args)
		{
			...
			string from = String.IsNullOrWhiteSpace(args.Parameters["from"]) ? "Server Admin" : args.Parameters["from"];
			player.SendInfoMessage("{0} just killed you!", from);
		...
		{
			string name = args.Parameters["group"];
		...
		private object GroupUpdate(RestRequestArgs args)
		{
			...
			Group group = (Group)ret;
			string parent = (null == args.Parameters["parent"]) ? group.ParentName : args.Parameters["parent"];
			string chatcolor = (null == args.Parameters["chatcolor"]) ? String.Format("{0}.{1}.{2}", group.R, group.G, group.B) : args.Parameters["chatcolor"];
			string permissions = (null == args.Parameters["permissions"]) ? group.Permissions : args.Parameters["permissions"];
			try
			{
		...
		{
			// This was used by TShock when automatically generating docs. It can be re-implemented as a command later.
